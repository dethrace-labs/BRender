                                            ...
                                            struct temp_face *tfp)
{
    ...
    if((clipped = FaceClip(rend.renderer, clip_in, rend.renderer->state.cache.clip_slots, tfp->codes, 3, &nclipped))) {
        ...
        if(tfp->flag & TFF_REVERSED) {
            ...
            for(i = 0; i < rend.renderer->state.cache.nconstant_fns; i++)
                rend.renderer->state.cache.constant_fns[i](rend.renderer, vp_p, vp_map, (br_vector3 *)fp_eqn, colour,
                                                           clipped->comp);
        }
...
void BR_ASM_CALL OpTriangleToLines(struct brp_block *block, ...)
{
    ...
    printf("OpTriangleToLines v0 %d, v1 %d v2 %d\n", v0->flags, v1->flags, v2->flags);
    /*
...
void BR_ASM_CALL averageVertices(struct br_renderer *renderer, brp_vertex *dest1, brp_vertex *dest2, brp_vertex *dest3,
                                 brp_vertex *src1, brp_vertex *src2, brp_vertex *src3);
...
static void triangleSubdivideCheck(int depth, struct brp_block *block, brp_vertex *v0, brp_vertex *v1, brp_vertex *v2,
                                   br_uint_16 (*fp_vertices)[3], br_uint_16 (*fp_edges)[3], br_vector4 *fp_eqn,
                                   struct temp_face *tfp);
static void triangleSubdivideOnScreen(int depth, struct brp_block *block, brp_vertex *v0, brp_vertex *v1,
                                      brp_vertex *v2, br_uint_16 (*fp_vertices)[3], br_uint_16 (*fp_edges)[3],
                                      br_vector4 *fp_eqn, struct temp_face *tfp);
