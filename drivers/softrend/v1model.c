...
static void GEOMETRY_CALL V1Face_Outcode(struct br_geometry *self, struct br_renderer *renderer)
{
    ...
    for(f = 0; f < rend.nfaces; f++, fp_vertices++, tfp++) {
         ...
         */
        combined_codes = rend.temp_vertices[(*fp_vertices)[0]].flags | rend.temp_vertices[(*fp_vertices)[1]].flags |
...
static br_error V1Model_Render(struct br_geometry *self, struct br_renderer *renderer, struct v11model *model,
                               struct br_renderer_state_stored *default_state, br_token type, br_boolean on_screen)
{
    int                              i, g;
    ...
    for(g = 0; g < model->ngroups; g++) {
        ...
        z_sort = (renderer->state.hidden.type == BRT_BUCKET_SORT && renderer->state.hidden.divert == BRT_ALL &&
                  renderer->state.hidden.order_table != NULL && scache.valid_primitive_heap);

        z_sort_blended = (renderer->state.hidden.type == BRT_BUCKET_SORT && renderer->state.hidden.divert == BRT_BLENDED &&
                          renderer->state.hidden.order_table != NULL && scache.valid_primitive_heap);

        r = PrimitiveStateRenderBegin(renderer->state.pstate, &rend.block, &rend.block_changed, &rend.range_changed,
                                      z_sort, type);
